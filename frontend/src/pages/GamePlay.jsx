 ...
 * Real gameplay logic will be in GameCanvas (not included here).
...
export default function GamePlay() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      // fetch level details (mock)
      const res = await fetch(`/api/levels/${levelId}`).catch(() => null);
      // use api.getLevel for consistent behavior
      // const res2 = await api.getLevel(levelId);
      const res2 = { title: `Level ${levelId}`, difficulty: Number(levelId) };
      if (!mounted) return;
      setLevelInfo(res2);
      setLoading(false);
    })();
  ...
  const handleLevelComplete = () => {
    // Update user progress locally and then call backend save
    updateProgress({ season: Number(seasonId), level: Number(levelId) });
    // navigate to level complete screen; pass stats via state if desired
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-900 to-gray-900 text-white p-4">
      <div className="max-w-5xl mx-auto">
        <div className="mb-4 flex justify-between items-center">
          ...
          <div>
            <button onClick={() => navigate(-1)} className="px-2 py-1 bg-gray-700 rounded">Exit</button>
          </div>
        </div>
        ...
        <div className="bg-white/5 rounded h-[420px] flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="mt-4 space-x-2">
              <button onClick={handleLevelComplete} className="px-3 py-2 bg-green-600 rounded">Simulate Win (Capture Flag)</button>
              <button onClick={() => { /* simulate death or retry */ }} className="px-3 py-2 bg-red-600 rounded">Simulate Death</button>
            </div>
          </div>
        </div>
